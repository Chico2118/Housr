...
export const register = async (req, res) => {
    try {
        ...
        if (!emailSent) {
            responseBody.warning = "User registered, but the verification email could not be sent.";
        }
...
export const forgotPassword = async (req, res) => {
    try {
        ...
        try {
            ...
            await user.save();
            return res.status(500).json({ message: "Could not send email", success: false, emailSent });
        }
    } catch (err) {
        res.status(500).json({ message: err.message, success: false });
    }
